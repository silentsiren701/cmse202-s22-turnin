...
class Universe:
    ...
    def move_particles(self):
        """move a particle, change it's a and y value"""
        for ip in self._all_particles:
            ip.move()

    def boundary_conditions(self):
        """check what's around this particle"""
        for ip in self._all_particles:
            if ip.x > self.width:
                ip.x -= self.width
    ...
    def draw(self):
        """use matplotlib to plot all the particle's position"""
    ...
    def check_interaction(self):
        """check the interaction between these particles."""
        ...
        for ip1, p1 in enumerate(self.fermions):
            for ip2, p2 in enumerate(self.fermions[ip1:]):
                if p1.x == p2.x and p1.y == p2.y and p1.is_antiparticle(p2):
                    ...
                    self.bosons.append(Boson(name="Boson", charge=0.0, mass=0.0,spin=1.0) )
